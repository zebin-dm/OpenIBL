...
def get_model(args):
    base_model = models.create(args.arch, train_layers=args.layers, cut_at_pooling=True,
    ...
    if args.vlad:
        ...
        initcache = osp.join(args.init_dir, args.arch + '_' +
                             args.dataset + '_' + str(args.num_clusters) + '_desc_cen.hdf5')
    ...
    model = nn.parallel.DistributedDataParallel(
        model, device_ids=[
            args.gpu], output_device=args.gpu, find_unused_parameters=False
    )
    return model
...
def main_worker(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.cache_size >= (len(dataset.q_train) // 3):
            args.cache_size = len(dataset.q_train) // 3
    ...
    #     osp.join(args.logs_dir, 'model_best.pth.tar'))['state_dict'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss-type', type=str,
                        default='triplet', help="[triplet|sare_ind|sare_joint]")
